import { readBoundedLocalTextFile, type LocalTextFile } from "../localFileImport";
...
export const CAD_IMPORT_MAX_BYTES = 100_000;
const CAD_IMPORT_CHARACTER_LIMIT = 100_000;
...
export function generateOpenScad(design: CadDesign): string {
  ...
  const header = [
    `// ${design.name}`,
    "// Generated by Engineering Mastery Lab CAD Studio",
    "// Units: millimetres",
    "$fn = 96;",
    ""
  ];
...
export function generateCadSvg(design: CadDesign): string {
  ...
  return [
    ...
    `<text x="42" y="594" font-family="Segoe UI, sans-serif" font-size="13" fill="#52657d">Generated by Engineering Mastery Lab CAD Studio. Verify before manufacture.</text>`,
    `</svg>`,
    ""
  ].join("\n");
}
...
export function importCadJson(source: string): CadDesign {
  if (source.length > CAD_IMPORT_CHARACTER_LIMIT) throw new Error("CAD design file is too large.");
...
export async function importCadFile(file: LocalTextFile): Promise<CadDesign> {
  const source = await readBoundedLocalTextFile(
    file,
    CAD_IMPORT_MAX_BYTES,
    "CAD design file"
  );
  return importCadJson(source);
}
